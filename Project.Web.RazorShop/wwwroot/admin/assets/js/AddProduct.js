...
$('#submit-form').on('click', function () {
    ...
    ajaxPostForm({
        ...
        var formIdGalleryInput = document.getElementById("formIdGallery");
        formIdGalleryInput.value = resp.data; // مقدار جدید که می‌خواهید اضافه شود
...
$('#Add_GaleyImage').on('click', function () {
    ...
    let ImageFormId = document.getElementById('formIdGallery');
    if (Image.files.length > 0) {
        formdata.set('Image', Image.files[0]);
    }
    formdata.set('ProductId', ImageFormId.value);
    ajaxPostForm({
        formData: formdata,
